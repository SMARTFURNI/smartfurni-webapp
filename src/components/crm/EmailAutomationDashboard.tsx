    ...
    <div className="space-y-6">
      ...
      {showSettings && (
        <div className="bg-[#1A1500] border border-[#2D2500] rounded-lg p-6">
          <h2 className="text-lg font-semibold text-white mb-4">Cron Job Settings</h2>
            ...
            <div className="flex items-center gap-4">
              <label className="flex items-center gap-2 cursor-pointer">
                <input
                  ...
                  className="w-4 h-4"
                />
                <span className="text-white">Enable Daily Email Automation</span>
              </label>
            </div>

            {cronEnabled && (
              <div>
                <label className="block text-sm text-[#9CA3AF] mb-2">Schedule Time (HH:MM)</label>
                <input
                  type="time"
                  value={cronTime}
      ...
      {stats && (
        <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
          <div className="bg-[#1A1500] border border-[#2D2500] rounded-lg p-4">
            <div className="text-[#9CA3AF] text-sm mb-1">Total Sent</div>
            <div className="text-2xl font-bold text-[#C9A84C]">{stats.totalSent}</div>
          </div>

          <div className="bg-[#1A1500] border border-[#2D2500] rounded-lg p-4">
            <div className="text-[#9CA3AF] text-sm mb-1">Open Rate</div>
            <div className="text-2xl font-bold text-[#22C55E]">{stats.openRate.toFixed(1)}%</div>
          </div>

          <div className="bg-[#1A1500] border border-[#2D2500] rounded-lg p-4">
            <div className="text-[#9CA3AF] text-sm mb-1">Click Rate</div>
      ...
      {logs.length > 0 && (
        <div className="bg-[#1A1500] border border-[#2D2500] rounded-lg overflow-hidden">
          <div className="px-6 py-4 border-b border-[#2D2500]">
            <h2 className="text-lg font-semibold text-white">Recent Email Logs</h2>
          </div>
            ...
            <table className="w-full">
              <thead>
                <tr className="bg-[#080600] border-b border-[#2D2500]">
                  <th className="px-6 py-3 text-left text-sm font-semibold text-[#9CA3AF]">Lead Name</th>
                  <th className="px-6 py-3 text-left text-sm font-semibold text-[#9CA3AF]">Email</th>
                  <th className="px-6 py-3 text-left text-sm font-semibold text-[#9CA3AF]">Subject</th>
                  <th className="px-6 py-3 text-left text-sm font-semibold text-[#9CA3AF]">Status</th>
                  <th className="px-6 py-3 text-left text-sm font-semibold text-[#9CA3AF]">Sent At</th>
                </tr>
              </thead>
              <tbody>
                {logs.map((log) => (
                  <tr key={log.id} className="border-b border-[#2D2500] hover:bg-[#080600]">
                    ...
                    <td className="px-6 py-3 text-sm text-[#9CA3AF]">{new Date(log.sentAt).toLocaleString()}</td>
                  </tr>
                ))}
              </tbody>
            </table>
      ...
      <div className="bg-[#1A1500] border border-[#2D2500] rounded-lg p-4">
        ...
        <ul className="text-sm text-[#9CA3AF] space-y-1">
          <li>✓ Automatically sends greeting emails to new leads daily</li>
          <li>✓ Customized templates based on lead information</li>
          <li>✓ Tracks open rates, click rates, and conversions</li>
          <li>✓ Integrates with Gmail SMTP for reliable delivery</li>
