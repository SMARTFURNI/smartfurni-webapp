...
export default function ZaloAutoReplyPanel({ onClose }: ZaloAutoReplyPanelProps) {
  ...
  return (
    <div className={`${sub.root} flex flex-col h-full bg-white dark:bg-white`}>
      ...
      {showCreate && (
        <div className={`${sub.formPanel} border-b border-gray-200 p-4 space-y-3`}>
          ...
          <div>
            ...
            <textarea value={message} onChange={e => setMessage(e.target.value)} rows={3} placeholder="Xin chào! Tôi hiện không có mặt. Tôi sẽ phản hồi sớm nhất có thể..." className="w-full px-3 py-2 text-sm border border-gray-200 dark:border-gray-200 rounded-lg bg-white dark:bg-gray-50 text-gray-900 dark:text-gray-900 focus:outline-none focus:ring-1 focus:ring-blue-500 resize-none" />
          </div>
          <div className="grid grid-cols-2 gap-2">
            <div>
              <label className="block text-xs text-gray-600 dark:text-gray-400 mb-1">Bắt đầu (tùy chọn)</label>
              <input type="datetime-local" value={startTime} onChange={e => setStartTime(e.target.value)} className="w-full px-2 py-1.5 text-xs border border-gray-200 dark:border-gray-200 rounded-lg bg-white dark:bg-gray-50 text-gray-900 dark:text-gray-900 focus:outline-none focus:ring-1 focus:ring-blue-500" />
            </div>
            <div>
              <label className="block text-xs text-gray-600 dark:text-gray-400 mb-1">Kết thúc (tùy chọn)</label>
              <input type="datetime-local" value={endTime} onChange={e => setEndTime(e.target.value)} className="w-full px-2 py-1.5 text-xs border border-gray-200 dark:border-gray-200 rounded-lg bg-white dark:bg-gray-50 text-gray-900 dark:text-gray-900 focus:outline-none focus:ring-1 focus:ring-blue-500" />
            </div>
          </div>
          <div className="flex gap-2">
            <button onClick={() => setShowCreate(false)} className="flex-1 py-1.5 text-xs text-gray-600 dark:text-gray-400 border border-gray-200 dark:border-gray-200 rounded-lg hover:bg-gray-50 dark:hover:bg-gray-100">Hủy</button>
                ...
                <div className="flex items-start justify-between gap-2">
                  <div className="flex items-center gap-2">
                    {r.isEnable ? (
                      ...
                      <span className="flex items-center gap-1 text-xs text-gray-400 bg-gray-100 dark:bg-gray-50 px-2 py-0.5 rounded-full">
                        <X size={10} /> Tắt
                      </span>
                    )}
                  </div>
                  <button onClick={() => handleDelete(r.id)} className="p-1 rounded hover:bg-red-50 dark:hover:bg-red-50 text-red-400 hover:text-red-600">
                    <Trash2 size={13} />
                  </button>
                </div>
      ...
      {/* Info */}
      <div className="px-4 py-3 border-t border-gray-100 dark:border-gray-100 bg-gray-50 dark:bg-gray-50">
