          ...
          {data.definition.steps.map(step => (
            <details key={step.id} className="group rounded-xl border border-gray-200 bg-gray-50 overflow-hidden">
              ...
              <div className="border-t border-gray-200 p-4 grid grid-cols-1 lg:grid-cols-2 gap-4 bg-white">
                <div>
                  <div className="flex items-center gap-2 text-xs font-semibold text-blue-700 mb-2"><MessageCircle size={13} /> Nội dung Zalo</div>
                  <pre className="whitespace-pre-wrap font-sans text-xs leading-5 text-gray-700 p-3 rounded-lg bg-blue-50/50 border border-blue-100">{step.zaloBody}</pre>
                </div>
                <div>
                  <div className="flex items-center gap-2 text-xs font-semibold text-violet-700 mb-2"><Mail size={13} /> Email: {step.emailSubject}</div>
                  <pre className="whitespace-pre-wrap font-sans text-xs leading-5 text-gray-700 p-3 rounded-lg bg-violet-50/40 border border-violet-100 max-h-96 overflow-auto">{step.emailBody}</pre>
                </div>
