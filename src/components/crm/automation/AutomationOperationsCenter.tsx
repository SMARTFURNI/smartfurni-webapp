...
export default function AutomationOperationsCenter() {
  ...
  return <div className="space-y-5">
    ...
    <section className="rounded-2xl border border-gray-200 bg-white p-5"><div className="mb-4 flex items-center gap-2"><MessageCircle size={18} className="text-blue-600" /><div><h3 className="text-sm font-bold text-gray-900">Nhóm tài khoản Zalo cá nhân</h3><p className="text-xs text-gray-500">Workflow ưu tiên hội thoại đã tương tác, sau đó tài khoản đã kết bạn; khách mới dùng tài khoản mặc định bên dưới và được giữ cố định khi xếp lịch.</p></div></div><div className="grid gap-3 md:grid-cols-2 xl:grid-cols-3">{data.zaloAccounts.map(account => <div key={account.id} className="rounded-xl border border-gray-200 p-3"><div className="flex items-center justify-between"><strong className="text-xs text-gray-900">{account.label || account.displayName}</strong><span className={`rounded-full px-2 py-1 text-[9px] font-bold ${account.isActive ? "bg-emerald-100 text-emerald-700" : "bg-gray-100 text-gray-500"}`}>{account.isActive ? "Hoạt động" : "Đã tắt"}</span></div><p className="mt-2 truncate text-[10px] text-gray-500">{account.displayName} · kết nối {dateLabel(account.lastConnected)}</p></div>)}{!data.zaloAccounts.length && <div className="rounded-xl border border-amber-200 bg-amber-50 p-3 text-xs text-amber-800">Chưa có tài khoản Zalo cá nhân hoạt động.</div>}</div></section>
      ...
      <div className="grid gap-3 md:grid-cols-2 xl:grid-cols-4">
        <label className="text-xs text-gray-600 md:col-span-2">Tài khoản mặc định cho kết bạn và workflow
          <select value={friendSettings.defaultAccountId} onChange={event => setFriendSettings({ ...friendSettings, defaultAccountId: event.target.value })} className="mt-1 w-full rounded-lg border border-gray-300 bg-white px-3 py-2">
            <option value="">Tự chọn tài khoản hoạt động đầu tiên</option>
            {data.zaloAccounts.filter(account => account.isActive).map(account => <option key={account.id} value={account.id}>{account.label || account.displayName}</option>)}
          </select>
          <span className="mt-1 block text-[10px] text-gray-400">Chỉ dùng khi khách chưa có hội thoại, chưa kết bạn và workflow chưa được ghim tài khoản.</span>
        </label>
        <label className="text-xs text-gray-600">Độ trễ hàng đợi dự phòng (phút)<input type="number" min={0} max={60} value={friendSettings.initialDelayMinutes} onChange={event => setFriendSettings({ ...friendSettings, initialDelayMinutes: Number(event.target.value) })} className="mt-1 w-full rounded-lg border border-gray-300 px-3 py-2" /></label>
        ...
        <div className="rounded-xl border border-blue-100 bg-white p-3 text-xs text-gray-600"><strong className="block text-gray-900">Gửi ngay có dự phòng</strong><span className="mt-1 block">Gửi ngay khi lưu · 72 giờ · gửi lại tối đa 2 lần · khung 08:30–19:30 · cron dự phòng 5 phút.</span></div>
