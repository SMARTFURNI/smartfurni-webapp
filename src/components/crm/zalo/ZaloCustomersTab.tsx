...
export default function ZaloCustomersTab({
  ...
  const stats = [
    [Users, "Tổng khách hàng", customers.length, "Từ Zalo OA", "blue"],
    [Tag, "Có tag Zalo OA", customers.filter(customer => customer.tagIds.some(id => tags.find(tag => tag.id === id)?.source === "oa")).length, `${tags.filter(tag => tag.source === "oa").length} tag đã đồng bộ`, "violet"],
    [UserRound, "Chưa phân nhóm", customers.filter(customer => !customer.tagIds.length).length, "Cần phân loại", "amber"],
    [FolderPlus, "Nhóm đang hoạt động", segments.length, "Nhóm động theo điều kiện", "emerald"],
  ] as const;

  return <div className="space-y-4">
    <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-4">{stats.map(([Icon, label, value, hint, tone]) => <div key={label} data-zalo-tone={tone} className={`${panel} p-4 transition hover:-translate-y-0.5 hover:shadow-[0_16px_34px_rgba(30,48,72,0.13)]`}><div className="flex items-center gap-3"><span className="flex h-10 w-10 items-center justify-center rounded-xl border border-[#c9a84c]/18 bg-[#c9a84c]/10 text-[#d6b75b]"><Icon size={18} /></span><div><div className="text-xs text-[rgba(245,237,214,0.48)]">{label}</div><strong className="mt-0.5 block text-2xl">{value.toLocaleString("vi-VN")}</strong></div></div><p className="mt-3 border-t border-white/6 pt-2 text-[11px] text-[rgba(245,237,214,0.34)]">{hint}</p></div>)}</div>
