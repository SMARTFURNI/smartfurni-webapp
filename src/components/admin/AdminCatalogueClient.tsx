...
export default function AdminCatalogueClient({ initialCatalogues }: Props) {
  ...
  return (
    <div className="min-h-screen bg-[#1a1200] text-white">
      {/* Header */}
      <div className="border-b border-[rgba(255,200,100,0.14)] px-6 py-4">
        <div className="flex items-center justify-between">
          <div>
            <h1 className="text-xl font-bold text-white">Catalogue B2B</h1>
            <p className="text-xs text-[rgba(245,237,214,0.55)] mt-0.5">
      ...
      <div className="px-6 py-6">
        ...
        {creating && (
          <div className="fixed inset-0 bg-black/70 backdrop-blur-sm z-50 flex items-center justify-center p-4">
            <div className="bg-[#111] border border-[rgba(255,200,100,0.22)] rounded-2xl p-6 w-full max-w-md">
              <h2 className="text-lg font-bold text-white mb-4">Tạo catalogue mới</h2>
              <div className="space-y-3">
                <div>
                  <label className="text-xs text-[rgba(245,237,214,0.70)] mb-1 block">Tên catalogue *</label>
                  <input
                    type="text"
                    value={newTitle}
                ...
                <div>
                  <label className="text-xs text-[rgba(245,237,214,0.70)] mb-1 block">Mô tả (tùy chọn)</label>
                  <textarea
                    value={newDesc}
        ...
        <div className="grid grid-cols-3 gap-4 mb-6">
          <div className="bg-[#111] border border-white/5 rounded-xl p-4">
            ...
            <div className="text-xs text-[rgba(245,237,214,0.55)] mt-0.5">Tổng catalogue</div>
          </div>
          <div className="bg-[#111] border border-white/5 rounded-xl p-4">
            <div className="text-2xl font-bold text-green-400">
              {catalogues.filter(c => c.status === "published").length}
            </div>
            <div className="text-xs text-[rgba(245,237,214,0.55)] mt-0.5">Đã xuất bản</div>
          </div>
          <div className="bg-[#111] border border-white/5 rounded-xl p-4">
            <div className="text-2xl font-bold text-[#C9A84C]">
              {catalogues.reduce((s, c) => s + c.viewCount, 0).toLocaleString()}
            </div>
            <div className="text-xs text-[rgba(245,237,214,0.55)] mt-0.5">Tổng lượt xem</div>
          </div>
        </div>
        ...
        {catalogues.length === 0 ? (
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4">
            {catalogues.map((cat) => (
              <div
                key={cat.id}
                className="bg-[#111] border border-white/5 rounded-xl overflow-hidden hover:border-[rgba(255,200,100,0.22)] transition-all group"
              >
                {/* Cover */}
                <div className="relative aspect-[3/2] bg-[#1a1200] overflow-hidden">
                  {cat.coverImageUrl ? (
                    <img
                  ...
                  <div className="absolute top-2 left-2">
                    <span className={`text-[10px] font-bold px-2 py-0.5 rounded-full ${
                      cat.status === "published"
                        ...
                        : "bg-gray-500/20 text-[rgba(245,237,214,0.70)] border border-gray-500/30"
                    }`}>
                ...
                <div className="p-4">
                  ...
                  {cat.description && (
                    <p className="text-[rgba(245,237,214,0.55)] text-xs line-clamp-2 mb-3">{cat.description}</p>
                  )}
                  <div className="flex items-center justify-between text-[11px] text-[rgba(245,237,214,0.45)] mb-3">
                  ...
                  <div className="flex gap-2">
                    ...
                    {cat.status === "published" && (
                      <a
                        ...
                        target="_blank"
                        className="w-8 h-8 flex items-center justify-center bg-white/5 text-[rgba(245,237,214,0.70)] rounded-lg hover:bg-white/10 hover:text-white transition-colors border border-white/5 text-xs"
