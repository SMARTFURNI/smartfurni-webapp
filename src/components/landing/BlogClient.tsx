...
export default function BlogClient({ theme, featured, allPosts }: Props) {
  ...
  return (
    <main className="min-h-screen bg-[#EEF0F1] font-sans text-[#20262E]">
      <Navbar theme={theme} />

      <section className="border-b border-[#C9C2B5] bg-[linear-gradient(180deg,#14110D_0,#14110D_72px,#F7F4EE_72px,#E9E4D9_100%)] px-4 pb-10 pt-28 sm:px-6 sm:pb-14 sm:pt-32">
        <ScrollReveal variant="fadeUp" delay={0}>
          <div className="mx-auto max-w-6xl text-center">
            ...
            <h1
              className="mx-auto max-w-4xl text-4xl font-light leading-[1.08] tracking-[-0.035em] text-[#1B222C] sm:text-5xl md:text-6xl"
      ...
      <section id="chuyen-muc" className="scroll-mt-24 border-b border-[#C8CDD2] bg-[linear-gradient(135deg,#E7EBEF_0%,#F1ECE2_100%)] px-4 py-8 sm:px-6 sm:py-10" aria-labelledby="blog-topics-heading">
        <div className="mx-auto max-w-6xl">
          <div className="mb-5 flex items-end justify-between gap-4">
            <div>
              <p className="text-[11px] font-bold uppercase tracking-[.18em] text-[#967129]">Khám phá nội dung</p>
              <h2 id="blog-topics-heading" className="mt-1 text-2xl font-light tracking-[-0.025em] text-[#1F2732] sm:text-3xl">Chuyên mục dành cho bạn</h2>
            </div>
            ...
            {(Object.keys(CATEGORIES) as BlogCategory[]).map((key) => {
              ...
              return (
                <button
                  ...
                  aria-pressed={active}
                  className={`rounded-2xl border p-4 text-left transition-all sm:p-5 ${active ? "-translate-y-0.5 shadow-[0_12px_32px_rgba(25,32,42,.15)] ring-1 ring-inset" : "shadow-[0_6px_22px_rgba(25,32,42,.06)] hover:-translate-y-0.5 hover:shadow-[0_12px_30px_rgba(25,32,42,.11)]"}`}
                  style={{
                    borderColor: active ? topic.accent : `${topic.accent}55`,
                    background: `linear-gradient(145deg, #FFFFFF 0%, ${topic.tint} 100%)`,
                    ...(active ? { boxShadow: `0 12px 32px ${topic.accent}24` } : {}),
                  }}
                >
                  <div className="flex items-center justify-between gap-2">
                    <span className="flex h-10 w-10 items-center justify-center rounded-xl text-xl text-white shadow-sm" style={{ backgroundColor: topic.accent }}>{topic.icon}</span>
                    <span className="rounded-full bg-white/80 px-2.5 py-1 text-xs font-semibold shadow-sm" style={{ color: topic.accent }}>{topicCounts[key]} bài</span>
                  </div>
                  <h3 className="mt-4 text-sm font-bold leading-5 text-[#202832] sm:text-base">{category.label}</h3>
                  <p className="mt-2 hidden text-xs leading-5 text-[#5F6873] sm:block">{topic.description}</p>
                </button>
              );
            })}
          </div>
      ...
      <div className="mx-auto max-w-6xl px-4 py-10 sm:px-6 sm:py-14">
        {!searchQuery && !activeCategory && leadPost && (
          <section className="mb-14 border-b border-[#D8D0C1] pb-14 sm:mb-16 sm:pb-16">
            <div className="mb-6 flex items-center justify-between border-b-2 border-[#2D2822] pb-3">
              <h2 className="text-2xl font-light tracking-[-0.025em] text-[#202832] sm:text-3xl">{theme.pageBlog.featuredTitle}</h2>
            ...
            <div className="grid gap-7 lg:grid-cols-[1.65fr_1fr]">
              <Link href={`/blog/${leadPost.slug}`} className="group grid overflow-hidden rounded-2xl border border-[#D8D0C1] bg-white shadow-[0_16px_45px_rgba(70,52,28,.08)] sm:grid-cols-[1.2fr_1fr] lg:grid-cols-1">
                ...
                <div className="p-5 sm:p-7">
                  ...
                  <h3 className="mt-3 text-2xl font-semibold leading-tight tracking-[-0.02em] text-[#202832] group-hover:text-[#8A6418] sm:text-3xl">{leadPost.title}</h3>
                ...
                {secondaryFeatured.map((post) => (
                  <Link key={post.slug} href={`/blog/${post.slug}`} className="group grid grid-cols-[116px_1fr] gap-4 py-5 first:pt-0 lg:grid-cols-1 lg:py-6 lg:first:pt-0">
                    ...
                    <div>
                      ...
                      <h3 className="mt-2 line-clamp-3 text-lg font-semibold leading-snug tracking-[-0.01em] text-[#202832] group-hover:text-[#8A6418] lg:text-xl">{post.title}</h3>
        ...
        <section id="danh-sach-bai-viet" className="scroll-mt-28">
          <div className="mb-6 flex flex-wrap items-end justify-between gap-4 border-b-2 border-[#2D2822] pb-3">
            <div>
              ...
              <h2 className="mt-1 text-2xl font-light tracking-[-0.025em] text-[#202832] sm:text-3xl">
          ...
          {filteredPosts.length > 0 ? (
              ...
              {filteredPosts.map((post) => (
                <Link key={post.slug} href={`/blog/${post.slug}`} className="group border-b border-[#D8D0C1] pb-7">
                  <article>
                    ...
                    <h3 className="mt-2 line-clamp-3 text-xl font-semibold leading-snug tracking-[-0.01em] text-[#202832] group-hover:text-[#8A6418]">{post.title}</h3>
            ...
            <div className="rounded-2xl border border-[#D8D0C1] bg-white py-16 text-center">
              ...
              <p className="mt-3 text-xl font-semibold text-[#202832]">Không tìm thấy bài viết</p>
        ...
        <section className="mt-14 overflow-hidden rounded-3xl bg-[linear-gradient(120deg,#182334,#302719)] p-7 text-center text-white shadow-[0_18px_50px_rgba(49,38,23,.14)] sm:p-10">
          ...
          <h3 className="mt-3 text-2xl font-semibold tracking-[-0.02em] sm:text-3xl">{theme.pageBlog.newsletterTitle}</h3>
