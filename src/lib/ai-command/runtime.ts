...
import { addMessage, createApproval, getSnapshot, updateRun } from "./store";
import { getApprovalPresentation } from "./tools";
import type { AiCommandActor, AiCommandSnapshot } from "./types";
...
export async function persistAgentResult(params: {
  ...
  const { result, runId, threadId, actor } = params;
  if (result.interruptions.length > 0) {
    ...
    await addMessage(
      ...
      { status: "awaiting_approval", runId, approvalIds: approvals.map(item => item.id) },
    );
  } else {
    ...
    await addMessage(threadId, "assistant", output, { status: "completed", runId });
    await logAudit({
